...
async function runNeonQuery(connectionString: string, sqlText: string): Promise<QueryResult> {
  ...
  const result = await (sql as any)([sqlText] as unknown as TemplateStringsArray);
        ...
        <div className="flex gap-2 mb-2">
          <div className="relative flex-1">
            <input
              ...
              className="w-full px-3 py-2.5 bg-[#141c0d] border border-gray-700/50 rounded-xl text-[12px] font-mono text-gray-200 placeholder-gray-700 outline-none focus:border-green-500/60 pr-16"
            />
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {QUICK_QUERIES.map(q => (
                    <button key={q.label} onClick={() => { setSql(q.sql); setTimeout(() => runQuery(q.sql), 50); }}
                      className="px-2 py-1.5 bg-[#141c0d] border border-gray-700/40 rounded-lg text-[10px] text-gray-300 hover:border-blue-500/40 hover:text-blue-300 text-left transition-all"
                    >{q.label}</button>
                  ))}
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {JURIDICO_TABLES.map(t => (
                    <button key={t.label} onClick={() => { setSql(t.sql); setTimeout(() => runQuery(t.sql), 50); }}
                      className="px-2 py-2 bg-[#141c0d] border border-amber-700/30 rounded-lg text-[10px] text-amber-200 hover:border-amber-500/60 text-left transition-all"
                    >{t.label}</button>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-[#0d1309] border border-[#2d4a1e] rounded-xl p-4">
                ...
                <div className="flex gap-2">
                  <select className="flex-1 px-3 py-2 bg-[#141c0d] border border-gray-700/50 rounded-xl text-[12px] text-gray-300 outline-none" onChange={async e => {
        ...
        <div className="bg-[#0d1309] border border-[#2d4a1e] rounded-xl p-4">
          ...
          <div className="flex gap-2 mb-3">
            <input value={dbName} onChange={e => setDbName(e.target.value)} placeholder="meu-banco.db"
              className="flex-1 px-3 py-2 bg-[#141c0d] border border-gray-700/50 rounded-xl text-[12px] font-mono text-gray-200 outline-none" />
          </div>
          <div className="flex gap-2">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {QUICK_QUERIES.map(q => (
                <button key={q.label} onClick={() => { setSql(q.sql); setTimeout(runQuery, 50); }}
                  className="px-2 py-1.5 bg-[#141c0d] border border-gray-700/40 rounded-lg text-[10px] text-gray-300 hover:border-blue-500/40 hover:text-blue-300 text-left"
                >{q.label}</button>
              ))}
            </div>
          </div>
      ...
      {showNew && (
        <div className="bg-[#0d1309] border border-[#2d4a1e] rounded-xl p-3 space-y-2">
          <input value={newLabel} onChange={e => setNewLabel(e.target.value)} placeholder="Nome (ex: Neon Jurídico)"
            className="w-full px-3 py-2 bg-[#141c0d] border border-gray-700/50 rounded-lg text-[12px] text-gray-200 outline-none focus:border-green-500/60" />
          <select value={newType} onChange={e => setNewType(e.target.value as any)}
            className="w-full px-3 py-2 bg-[#141c0d] border border-gray-700/50 rounded-lg text-[12px] text-gray-300 outline-none">
          ...
          <input value={newConn} onChange={e => setNewConn(e.target.value)} placeholder="postgresql://usuario:senha@host/banco"
            type="password"
            className="w-full px-3 py-2 bg-[#141c0d] border border-gray-700/50 rounded-lg text-[12px] font-mono text-gray-200 outline-none focus:border-green-500/60" />
          <div className="flex gap-2">
        ...
        <div className="bg-[#1a2413] border-t border-gray-700/50 rounded-t-3xl shadow-2xl flex flex-col" style={{ maxHeight: "92vh" }}>
          ...
          <div className="flex items-center justify-between px-5 py-2.5 border-b border-gray-700/40 shrink-0">
            <div className="flex items-center gap-2">
              ...
              {connectionString && <span className="text-[10px] text-green-400 bg-green-900/30 border border-green-700/40 px-2 py-0.5 rounded-full font-bold">✓ Configurado</span>}
            </div>
